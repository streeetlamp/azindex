...
* WordPress 5.0+ recommended; WordPress 2.5 is the minimum required
* PHP 7.4 minimum; tested against PHP 8.1 and PHP 8.2
...
**Changes in version 0.8.2 (PHP 8.2 compatibility — VCU Libraries maintenance)**

This release eliminates PHP 8.x deprecation warnings and undefined-variable notices that appeared
in WP debug logs and on the admin Tools page. No index behavior, output, or shortcode API has changed.

*az-index-admin.php*

* Added `__construct()` bridge for `az_request` so PHP 8 no longer treats the same-name method as
  a constructor (deprecated in PHP 7, removed in PHP 8).
* Added `attribute_escape()` polyfill backed by `esc_attr()` for WordPress 6.x compatibility.
* Removed dead `$wp_version` conditional for `AZ_PAGENAME`; hardcoded `tools.php` (WP 2.7 is ancient).
* Replaced user-level integer `8` with `'manage_options'` capability string in
  `add_management_page()` (deprecated since WordPress 2.0).
* Initialized `$upgrade` and `$charset` before conditional blocks to silence PHP 8 undefined-variable warnings.
* Replaced `$wpdb->supports_collation()` (deprecated WP 3.5) with `$wpdb->has_cap('collation')`.
* Fixed `$create` typo → `$created` in `az_plugin_activate()`.
* Fixed action detection in `az_request()` — now reads `$_POST['action']` / `$_GET['action']`
  instead of the non-existent `$_POST['options']`, restoring correct Tools-page render state.
* Guarded all `set_vars_from_get()` accesses for `indexid`, `name`, `msg`, and `by` with `isset()`
  defaults so missing GET parameters no longer trigger undefined-index notices.
* Fixed undefined variable `$error_field` in the add/edit dialog; all accesses now go through
  `$req->error_field` consistently (including the `desckey` field that was previously bare).

*az-index-cache.php*

* Added `__construct()` bridges for `az_index_cache`, `az_mutex`, and `az_terms` classes.
* Fixed `$pos` undefined-constant typo in `process_dirty_items()`.
* Replaced `utf8_decode()` (deprecated PHP 8.2, removed PHP 9.0) with
  `mb_convert_encoding(..., 'ISO-8859-1', 'UTF-8')` in the Windows NLS path.
* Fixed invalid `preg_match` delimiter: `'[[:alnum:]]'` → `'/[[:alnum:]]/'` in the
  `az_compare()` non-multibyte branch.
* Added defensive initialization for `$keys`, `$output`, and `$previnitial` before loops to
  prevent undefined-variable warnings on empty or partially-populated indexes.
* Added `isset()` guards on `include`/`exclude` term-array keys before accessing them in filter loops.
* Added null-checks around dirty-item iteration and custom-field offset accesses.

*az-index-content.php*

* Added defensive initialization for `$previnitial`, alpha-links output accumulators (`$output`),
  and page-links output buffer to eliminate undefined-variable warnings during index rendering.
