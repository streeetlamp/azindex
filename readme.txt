...
**Changes in version 1.0.0 (PHP 8.x compatibility patch — VCU Libraries maintenance)**

This release eliminates the PHP 8.x deprecation/warning spam that appeared in WP debug logs and on
the admin Tools page. No index behavior or output has changed.

*az-index-admin.php*

* Guarded all `set_vars_from_get()` accesses for `indexid`, `name`, `msg`, and `by` with `isset()`
  defaults so that missing GET parameters no longer trigger undefined-index notices.
...
* Fixed undefined variable `$error_field` in the add/edit index dialog; accesses now go through
  `$req->error_field` consistently (including the `desckey` field that was previously bare).

*az-index-cache.php*

* Added defensive initialization for `$keys`, `$output`, and `$previnitial` to prevent
  undefined-variable warnings when cache-build routines run on an empty or partially-populated index.
* Added `isset()` guards on `include`/`exclude` term-array keys before accessing them in filter loops.
* Added null-checks around dirty-item iteration loops so the cache invalidation path no longer
  emits warnings when the dirty list is `null`.
* Added null/offset guards on custom-field value accesses that could receive `null` from
  `get_post_meta()` on posts with no matching custom field.

*az-index-content.php*

* Mirrored the same defensive `$keys`, `$output`, and `$previnitial` initialization fixes from
  the cache file into the content-rendering path.
